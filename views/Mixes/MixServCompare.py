...
def viewMixServCompare(request,mix,option):
    ...
    for guild in listeMixs:
        try:
            ...
            if option in ("messages","voice","mots"):
                ligne=curseur.execute("SELECT SUM(Count) AS Count FROM {0}{1}".format(moisDB1,anneeDB1)).fetchone()
            else:
                ligne=curseur.execute("SELECT * FROM {0}{1} WHERE ID={2}".format(moisDB1,anneeDB1,obj)).fetchone()
            ligne["Nom"]=guild["Nom"]
            ligne["ID"]=guild["ID"]
    ...
    for guild in listeMixs:
        try:
            ...
            if option in ("messages","voice","mots"):
                ligne=curseur.execute("SELECT SUM(Count) AS Count FROM {0}{1}".format(moisDB2,anneeDB2)).fetchone()
            else:
                ligne=curseur.execute("SELECT * FROM {0}{1} WHERE ID={2}".format(moisDB2,anneeDB2,obj)).fetchone()
            ligne["Nom"]=guild["Nom"]
            ligne["ID"]=guild["ID"]
            ligne["Icon"]=guild["Icon"]
            maxi=max(maxi,ligne["Count"])
    ...
    "lisPlus":["serv","perso","compare","compareperso"],"dictPlus":dictRefPlus,"plus":"compare",
    "travel":False,"selector":True,"obj":obj,"listeObjs":listeObj,"mix":True,"listeMix":listeMixs,"nbmix":infosMix["Nombre"],
    "pin":getPin(user,curseurGet,"mixes/{0}".format(infosMix["Nombre"]),option,"periods","compare")}
